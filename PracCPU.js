$(document).ready(
    function(){
        ...
        $('#btnAddProcess').on('click', function(){
           ...
           // var lastRow = $('#tblProcessList tr:last');
 // var lastRowNumebr = parseInt(lastRow.children()[1].innerText);

//   var newRow = '<tr><td><input class="form-control" type="number" min="0" id="processID" ></td>'
//   + '</td><td><input class="form-control mb-2" type="number" min="0" id="arrivalTime" ></td>'
//   + '<td><input class="form-control mb-2" type="number" min="0" id="burstTime"></td></tr>'
        ...
        function firstComeFirstServed(){
            ...
            $.each(completedList, function(key, process){
                $('#tblResults > tbody:last-child').append(
                    `<tr>
                        <td id="tdProcessID"><!--<input class="form-control" id="tdProcessID" type="number" placeholder="0" enable>-->${process.processID}</td>
                        ${$('#tdProcessID').val()!=process.processID?showModal():""}
                        ${console.log($('#tdProcessID').val())}
...
console.log(tempp);
            // Get average
            var useravgWaitingTime;
            var useravgTurnaroundTime;
            ...
            var aTAT=avgTurnaroundTime / completedList.length ;
            var raTAT=roundToTwo(aTAT);
            console.log(raTAT);
            useravgTurnaroundTime=$('#avgTurnaroundTime').val();
            if(raTAT!=useravgTurnaroundTime)
            {
                msgValue=`<p>Oops! Average Turnaround Time is wrong.<br> Please check the turnaround time of each process, total turnaround time and then average turnaround time.<br>
                TAT=CT-AT<br>
                Where TAT is turn around time,<br>
                CT is Completion Time and<br>
                AT is Arrival Time<br>
                For more refer to this :<a href="https://www.geeksforgeeks.org/cpu-scheduling-in-operating-systems/" target="_blank">Click Here</a></p>`;
                showModal();
            }
            else{
            //console.log("NO");
            var aWT=avgWaitingTime / completedList.length
            var raWT=roundToTwo(aWT);
            console.log(raWT);
            useravgWaitingTime=$('#avgWaitingTime').val();
            if(raWT!=useravgWaitingTime){
                msgValue=`<p>Oops! Average Waiting Time is wrong.<br> Please check the waiting time of each process, total waiting time and then average waiting time.<br>
                WT=TAT-BT<br>
                Where TAT is turn around time,<br>
                WT is Completion Time and<br>
                BT is Arrival Time<br>
                For more refer to this :<a href="https://www.geeksforgeeks.org/cpu-scheduling-in-operating-systems/" target="_blank">Click Here</a></p>`;
                showModal();
            }
            else{
...
//to open gannt chart window
function showModal(){
    let modal=document.getElementsByClassName("modal")[0]
    modal.style.display="block";
   // document.getElementById("msg").innerHTML="Please check your value.ATAT is wrong";
    showMsg();
}
